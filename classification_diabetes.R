...
# corr_mat <- cor(diabetes[,1:8])
# corrplot(corr_mat, order = "hclust", tl.cex = 1, addrect = 8)

# cleaning data to filter out impractical data
...
# rpart plot plots an rpart model
...
ggplot(diabetes_clean, aes(x = Glucose,fill=Outcome)) + 
  geom_bar(position="Dodge") + 
  ggtitle("Distribution of Glucose amount")
